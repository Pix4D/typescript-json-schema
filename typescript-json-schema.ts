...
export class JsonSchemaGenerator {
  ...
  private parseCommentsIntoDefinition(
    ...
    jsdocs.forEach(doc => {
      ...
      const [name, text] = (doc.name === 'TJS'
        ? new RegExp(REGEX_TJS_JSDOC).exec(doc.text!)!.slice(1, 3)
        : [doc.name, doc.text]) as string[];
  ...
  private extractLiteralValue(typ: ts.Type): PrimitiveType | undefined {
    if (typ.flags & ts.TypeFlags.EnumLiteral) {
      ...
      return parseFloat(String((<ts.LiteralType>typ).value));
  ...
  private getDefinitionForRootType(
    ...
    if (tupleType) {
      // tuple
      const elemTypes: ts.NodeArray<ts.TypeNode> =
        tupleType.elementTypes || (propertyType as any).typeArguments;
      ...
      const propertyTypeString = tc.typeToString(
        propertyType,
        undefined,
      ...
      switch (propertyTypeString.toLowerCase()) {
        ...
        case 'number':
          const isInteger =
            definition.type === 'integer' || (reffedType && reffedType.getName() === 'integer');
        ...
        default:
          ...
          if (value !== undefined) {
            ...
            const typeArguments = (<ts.TypeReference>propertyType).typeArguments;
            if (typeArguments) {
  ...
  private getDefinitionForProperty(prop: ts.Symbol, tc: ts.TypeChecker, node: ts.Node) {
    ...
    let definition = this.getTypeDefinition(
      propertyType,
      tc,
      undefined,
      undefined,
  ...
  private getEnumDefinition(
    ...
    const fullName = tc.typeToString(
      clazzType,
      undefined,
      ts.TypeFormatFlags.UseFullyQualifiedType,
    );
  ...
  private getTypeDefinition(
    ...
    if (!asRef || !this.reffedDefinitions[fullTypeName]) {
      if (asRef) {
        ...
        this.reffedDefinitions[fullTypeName] =
          asTypeAliasRef && reffedType!.getFlags() & ts.TypeFlags.IndexedAccess && symbol
            ? this.getTypeDefinition(typ, tc, true, undefined, symbol, symbol)
            : definition;
      ...
      const node =
        symbol && symbol.getDeclarations() !== undefined ? symbol.getDeclarations()[0] : null;
  ...
  public getSchemaForSymbols(symbols: string[]): Definition {
    ...
    for (let i = 0; i < symbols.length; i++) {
      ...
      root.definitions[symbol] = this.getTypeDefinition(
        this.userSymbols[symbol],
        this.tc,
        this.args.topRef,
      );
    }
...
export function run() {
  ...
  var args = require('yargs')
    ...
    .default('defaultProps', defaultArgs.defaultProps)
    ...
    .describe('validationKeywords', 'Provide additional validation keywords to include.').argv;
